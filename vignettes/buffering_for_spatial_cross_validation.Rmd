...
One special case, however, is when `buffer` is set to 0. In this case, spatialsample won't apply a buffer at all. While polygons that share an edge are within 0 distance of each other, when calculated from edge-to-edge, we think that setting `buffer = 0` would intuitively apply 0 (that is, "no") buffer. If you want to be sure to only capture adjacent polygons in a buffer, set `buffer` to a tiny, non-0 value:
