...
that our `buffer` did anything. However, if we plot each `rsplit` object separately, we can see that we've created a small exclusion buffer around each test (otherwise known as assessment) set:

```{r buffer-gif, animation.hook="gifski", fig.width=7, fig.height=5}
purrr::walk(buffer_folds$splits, function(x) print(autoplot(x)))
```

Points inside of this buffer are included in neither the analysis nor assessment sets, making sure you're enforcing a minimum amount of spatial extrapolation when predicting on new data. For more information on this process, check out the [buffering vignette](https://spatialsample.tidymodels.org/dev/articles/buffering.html)!
